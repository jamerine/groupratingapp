  ...
  def self.perform(group_rating_id)
    ...
    ProcessPayrollAllTransactionsBreakdownByManualClass.find_each do |payroll_transaction|
      ...
      unless @manual_class_calculation.nil?
        PayrollCalculation.where(policy_number: payroll_transaction.policy_number, manual_number: payroll_transaction.manual_number, representative_number: payroll_transaction.representative_number, manual_class_calculation_id: @manual_class_calculation.id).update_or_create(
        ...
        payroll_origin: payroll_transaction.manual_class_payroll,
        data_source: payroll_transaction.manual_class_payroll
        )
      end
    end
    @group_rating = GroupRating.find_by(id: group_rating_id)
