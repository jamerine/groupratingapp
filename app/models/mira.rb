...
class Mira < ActiveRecord::Base
  scope :by_record_type, -> { where("substring(miras.single_rec,10,2) = '02'") }
  scope :by_representative, -> (rep_number) { where("cast_to_int(substring(miras.single_rec,1,6)) = ?", rep_number) }
