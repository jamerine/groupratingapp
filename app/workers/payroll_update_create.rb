class PayrollUpdateCreate
  ...
  def self.perform(group_rating_id)
    ...
    @group_rating.save
    ProcessPayrollAllTransactionsBreakdownByManualClass.where("manual_class_effective_date > ?", @group_rating.experience_period_lower_date).find_each do |payroll_transaction|
