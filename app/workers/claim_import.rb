  ...
  def perform(claim_hash)
    ...
    if account
      ...
      if policy_calculation
        @claim_calculation = ClaimCalculation.where(representative_number: representative.representative_number, policy_number: policy_calculation.policy_number, policy_calculation_id: policy_calculation.id, data_source: 'user', claim_number: "#{claim_hash['claim_number'].strip}").update_or_create(claim_hash.except("representative_number", "policy_number", "claim_number"))
