class ClaimCalculationsController < ApplicationController
  ...
  def search
    ...
    @claim_search_results = ClaimCalculation.includes(policy_calculation: [:account, :representative]).where("claim_number ILIKE :search_value OR REPLACE(claim_number, '-', '') ILIKE :search_value OR claimant_name ILIKE :search_value", search_value: "%#{@search_value}%").order('accounts.name ASC').order(:policy_number)
