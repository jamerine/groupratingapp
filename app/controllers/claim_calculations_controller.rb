class ClaimCalculationsController < ApplicationController
  ...
  def search
    ...
    @claim_search_results = ClaimCalculation.includes(:policy_calculation, :representative)
                            .joins(policy_calculation: :account)
                            .where("accounts.representative_id IN (?)", current_user.representatives_users.pluck(:representative_id).compact)
                            .includes(policy_calculation: [:account, :representative])
                            .where("claim_calculations.claim_number ILIKE :search_value OR REPLACE(claim_calculations.claim_number, '-', '') ILIKE :search_value OR claim_calculations.claimant_name ILIKE :search_value OR accounts.name ILIKE :search_value", search_value: "%#{@search_value}%")
                            .order("accounts.name ASC")
                            .order("claim_calculations.policy_number")
