class Step300Proc2 < ActiveRecord::Migration
  def up
    connection.execute(%q{
      -- Function: public.proc_step_300(integer, date, date, date)

  -- DROP FUNCTION public.proc_step_300(integer, date, date, date);

  CREATE OR REPLACE FUNCTION public.proc_step_300(
  ...
  $BODY$
        ...
        -- ADDED THIS ON 06/20/2017 to delete the possibility of duplicate payroll records from pcomb file.  Deletes the duplicate and leaves the larger payroll value.

        DELETE FROM public.pcomb_detail_records
            WHERE id IN (SELECT id
               FROM (SELECT id, ROW_NUMBER() OVER (partition BY
               ...
               payroll_reporting_period_to_date
                           ORDER BY manual_payroll DESC) AS rnum
  ...
  def down
    connection.execute(%q{
      DROP FUNCTION public.proc_step_300(integer, date, date, date, date);
    })
  end
end
